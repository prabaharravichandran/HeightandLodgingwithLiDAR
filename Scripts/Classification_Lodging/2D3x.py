...
from sklearn.metrics import f1_score  # Import Macro-F1 scoring function
...
for document in tqdm(valid_documents, desc="Processing valid documents"):
    try:
        ...
        resized_grid = projection3x(lidar_raw)
...
x_train_data = np.reshape(np.asarray(x_train), (len(x_train), 300, 100, 3))
y_train_data = np.reshape(np.asarray(y_train), (len(y_train), 1))

print(f"Training observations: {x_train_data.shape[0]}")
...
def prepare_model_without_output(input_shape):
    ...
    def residual_depthwise_separable_block(x, filters, kernel_size, strides):
        shortcut = x
        x = layers.DepthwiseConv2D(kernel_size=(kernel_size, kernel_size), strides=(strides, strides), padding='same')(
            x)
        x = layers.LayerNormalization()(x)
        x = layers.ReLU()(x)
        x = layers.Conv2D(filters, kernel_size=(1, 1), padding='same', activation='relu')(x)
        x = layers.LayerNormalization()(x)
        if shortcut.shape == x.shape:
            x = layers.Add()([shortcut, x])
        else:
            shortcut = layers.Conv2D(filters, kernel_size=(1, 1), strides=(strides, strides), padding='same')(shortcut)
            x = layers.Add()([shortcut, x])
        x = layers.ReLU()(x)
        return x
    ...
    x = residual_depthwise_separable_block(x, 1028, kernel_size=3, strides=1)
...
with strategy.scope():
    base_model = prepare_model_without_output(input_shape=(300, 100, 3))
...
for rep in range(1, 26):  # Run repetitions
    ...
    for iteration in range(1, 5):  # Iterations for progressive fine-tuning
        ...
        X_train_resampled = X_train_resampled.reshape(-1, X_train_raw.shape[1], X_train_raw.shape[2], 3)
        ...
        train_generator = datagen.flow(X_train_final, Y_train_final, batch_size=16)
        ...
        if (
            ...
            model_filename = f"{model_save_path}best_model_for_2D3x_iteration_{iteration}.h5"
    ...
    output_file = f'{model_save_path}2D3x_all_results.xlsx'
