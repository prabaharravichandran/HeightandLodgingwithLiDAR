...
for rep in range(1, 26):  # Run repetitions
    ...
    for iteration in range(1, 5):  # Iterations for progressive fine-tuning
        ...
        # Step 2: Perform a single split for train and test sets
        train_indices, test_indices = train_test_split(
            np.arange(len(x_train_data)), test_size=0.2, shuffle=True
        )
        X_train_raw, X_test = x_train_data[train_indices], x_train_data[test_indices]
        Y_train_raw, Y_test = y_train_mapped[train_indices], y_train_mapped[test_indices]
    ...
    output_file = f'{model_save_path}1D_all_results.xlsx'
